...
* Check the `ConnectionStrings` in `appsettings.json` files under the `Dna.Sai.AuthServer`, `Dna.Sai.HttpApi.Host` and `Dna.Sai.DbMigrator` projects and change it if you need.
* Check the `ConnectionStrings` in `appsettings.json` files under the `Dna.Sai.Web.Public` as well.
...
* Run `Dna.Sai.DbMigrator` to create the initial database. This should be done in the first run. It is also needed if a new database migration is added to the solution later.
...
```bash
dotnet dev-certs https -v -ep authserver.pfx -p bbe32312-4799-4222-bbdc-665ae2e460cc
```

> `bbe32312-4799-4222-bbdc-665ae2e460cc` is the password of the certificate, you can change it to any password you want.
...
* `Dna.Sai.DbMigrator`: A console application which applies the migrations and also seeds the initial data. It is useful on development as well as on production environment.
* `Dna.Sai.AuthServer`: ASP.NET Core MVC / Razor Pages application that is integrated OAuth 2.0(`OpenIddict`) and account modules. It is used to authenticate users and issue tokens.
* `Dna.Sai.HttpApi.Host`: ASP.NET Core API application that is used to expose the APIs to the clients.
* `Dna.Sai.Web.Public`: ASP.NET Core MVC / Razor Pages application that is the public web application of the solution.
