...
public class LoginService : ILoginService, ITransientDependency
{
    ...
    {
        var token = await _storage.GetAsync(SaiConsts.OidcConsts.AccessTokenKeyName);
    ...
    {
        var refreshToken = await _storage.GetAsync(SaiConsts.OidcConsts.RefreshTokenKeyName);
    ...
    {
        await _storage.SetAsync(SaiConsts.OidcConsts.AccessTokenKeyName, accessToken);
        await _storage.SetAsync(SaiConsts.OidcConsts.RefreshTokenKeyName, refreshToken);
    }

    private async Task ClearTokenCacheAsync()
    {
        await _storage.RemoveAsync(SaiConsts.OidcConsts.AccessTokenKeyName);
        await _storage.RemoveAsync(SaiConsts.OidcConsts.RefreshTokenKeyName);
    }
}